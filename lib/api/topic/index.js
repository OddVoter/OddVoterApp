...
app.get('/topic/:id/report', 
  ...
  function (req, res) {
    ...
    if (type === 'raw') {
      Topic.getTopicResultsReport(topicId, function(data) {
        return res.status(200).send(data)
      })
    } else if (type === 'status') {
      Topic.getTopicSummaryStatus(topicId, function(data) {
        return res.status(200).send(data)
      })
    } else if (type === 'district') {
      Topic.getTopicSummaryMunicipalDistrict(topicId, function(data) {
        return res.status(200).send(data)
      })
    } else if (type === 'date') {
      Topic.getTopicSummaryLastVotedDate(topicId, function(data) {
        return res.status(200).send(data)
      })
    }
})
