...
exports.getByEmail = function search (email, fn) {
  ...
  User.findOne({ email: email })
    .select('firstName lastName fullName email avatar profilePictureUrl notifications emailValidated extra homeAddress dateOfBirth voterMatchOptIn voterMatched hideMatchMessage')
    .exec(function (err, user) {
      if (err) {
        log('Found error: %j', err)
        return fn(err)
 ...
 *
 * @param {ObjectId|String} data to update user
 * @param {Function} fn callback function
