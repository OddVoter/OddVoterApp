...
var TopicReport = require('lib/utils/reports')
...
TopicSchema.methods.votedBy = function (user) {
   if (!user || (this.action.boxCount === 0)) return false

   const votes = this.action.box
...
TopicSchema.methods.vote = function (user, value, cb) {
  ...
  if (!!this.votedBy(user)) return cb(new Error('Already voted.'))

  const vote = { author: user, value: value }

  if (this.action.box) {
    this.action.box.push(vote)
  } else {
    this.action.box = [vote]
  }

  this.action.boxCount = this.action.box.length

  const votes = this.action.box
  const options = this.action.results.map(o => o.value)
  ...
  this.action.results = options.map((opt) => ({
    value: opt,
...
/**
 * Get CSV breakdown of topic results including their associated registered voter record, if any.
 *
 * @param {String} topicId
 * @api public
 */
TopicSchema.statics.getTopicResultsReport = function (topicId, cb) {
  this.findById(topicId)
  .exec()
  .then((topic) => {
    if (topic.action !== null) {
      TopicReport.getRawTopicResults(topic, function(data) {
        cb(data);
      })
    }
  })
...
/**
 * Get topic and aggregate results based on if it's a vote or poll
 *
 * @param {objectId} topicId 
 * @api public
 */

TopicSchema.statics.getTopicSummaryStatus = function (topicId, cb) {
  this.findById(topicId)
  .exec()
  .then((topic) => {
    if (topic !== null && topic.action !== null) {
      if (topic.action.method === 'poll') {
        TopicReport.getTopicSummaryStatusPoll(topic, function(data) {
          cb(data);
        })
      } else if (topic.action.method === 'vote') {
        TopicReport.getTopicSummaryStatusVote(topic, function(data) {
          cb(data);
        })
      }
    }
...
/**
 * Get topic voter summary by municipal district
 *
 * @param {objectId} topicId 
 * @api public
 */

TopicSchema.statics.getTopicSummaryMunicipalDistrict = function (topicId, cb) {
  this.findById(topicId)
  .exec()
  .then((topic) => {
    if (topic !== null && topic.action !== null) {
      if (topic.action.method === 'poll') {
        TopicReport.getTopicSummaryDistrictPoll(topic, function(data) {
          cb(data);
        })
      } else if (topic.action.method === 'vote') {
        TopicReport.getTopicSummaryDistrictVote(topic, function(data) {
          cb(data);
        })
      }
    }
...
/**
 * Get topic voter summary by last voted date
 *
 * @param {objectId} topicId 
 * @api public
 */

TopicSchema.statics.getTopicSummaryLastVotedDate = function (topicId, cb) {
  this.findById(topicId)
  .exec()
  .then((topic) => {
    if (topic !== null && topic.action !== null) {
      if (topic.action.method === 'poll') {
        TopicReport.getTopicSummaryDatePoll(topic, function(data) {
          cb(data);
        })
      } else if (topic.action.method === 'vote') {
        TopicReport.getTopicSummaryDateVote(topic, function(data) {
          cb(data);
        })
      }
    }
