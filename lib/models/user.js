...
const UserSchema = new Schema({
  ...
  county: { type: String },
  voterMatchOptIn: { type: Boolean },
  voterMatched: { type: Boolean },
  voterRecord: { type: Object },
  voterRegistration: { type: String },
  voterNeedsReview: { type: Boolean },
  hideMatchMessage: { type: Boolean },
...
/**
 * Find 'User' records where the user opted in to have their voter record matched
 */

UserSchema.statics.findOptInRecords = function () {
  return this.find({ 'voterMatchOptIn' : true }, { firstName: 1, lastName: 1, county: 1, dateOfBirth: 1 }).exec()
}

/**
 * Update user by adding their matched voter record object
 * NOTE: This is no longer used since we now store voterRegistration ID for user, not the voter record object.
 *
 * @param {String} userId
 * @param {objectId} voterRecord 
 * @api public
 */

UserSchema.statics.updateUserMatchedVoterRecord = function (userId, voterRecord) {
  return this
    .findById(userId)
    .exec()
    .then((user) => {
      if (user !== null) {
        if (voterRecord !== undefined) {
          user.voterRecord = voterRecord
          user.voterMatched = true
          user.voterNeedsReview = false // We are assuming there is a correct 1 to 1 match
          user.save()
        }
      }
    })
...
/**
 * Update user by adding their matched voter registration ID
 * 
 * @param {String} userId
 * @param {objectId} voterRegistration 
 * @api public
 */

UserSchema.statics.updateUserMatchedVoterRegistration = function (userId, voterRegistration) {
  return this
    .findById(userId)
    .exec()
    .then((user) => {
      if (user !== null) {
          user.voterRegistration = voterRegistration
          user.save()
      }
    }) 
}
...
/**
 * Update user by flagging them for review (multiple or nomatches)
 *
 * @param {String} userId
 * @api public
 */

UserSchema.statics.updateUserNeedsReview = function (userId) {
  return this
    .findById(userId)
    .exec()
    .then((user) => {
...
/**
 * Determine if user is a registered voter
 *
 * @param {String} userId
 * @api public
 */

UserSchema.statics.isRegisteredVoter = function (userId, cb) {
  this.findById(userId)
  .exec()
  .then((user) => {
    if (user !== null) {
      if (cb) {
        return cb(user.voterRegistration !== undefined ? true : false)
      }
      return user.voterRegistration !== undefined ? true : false
    }
  })
}
