...
var privileges = {
  canEdit: function canEdit (forum, user, topic) {
    ...
    const isAdmin =
      forum.isOwner(user) ||
      forum.hasRole(user, 'admin', 'collaborator', 'representative')

    if (forum.hasVisibility('collaborative')) {
      if (isAdmin && forum.hasRole(topic.owner, 'admin', 'collaborator', 'representative', 'author')) return true
  ...
  canDelete: function canDelete (forum, user, topic) {
    ...
    if (forum.hasVisibility('representative') && topic.isOwner(user)) return true
    return false
  }
}
