...
app.get('/auth/facebook/callback',
  ...
  function (req, res) {
    ...
    // Check to see if the user has already given us their date of birth. If so,
    // then redirect the user to homepage. Otherwise, redirect to sign-up page
    if (req.user && req.user.dateOfBirth && req.user.homeAddress) {
      return res.redirect('/')
    }
    else {
      return res.redirect('/fb-signup')
    }
  }
)
