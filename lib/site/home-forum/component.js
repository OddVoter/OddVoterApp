...
export class HomeForum extends Component {
  ...
  componentDidMount() {
    if (this.props.user.state.fulfilled) {
      ...
      this.getVoterMatchResult(user)
    }
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.user.state.fulfilled) {
      const user = nextProps.user.state.value
      this.getVoterMatchResult(user)
    }
    else {
      this.setState({
  ...
  getVoterMatchResult(user) {
    ...
    if (user.voterMatchOptIn) {
      this.setState({
        hideVoterMatchMessage: user.hideMatchMessage,
        voterMatched: user.voterMatched ? true : false
      })
    }
  }

  updateUserHideMessage() {
    Request
    .put(`/api/user/${this.state.user.id}`)
    .send({
  ...
  render () {
    ...
    else if (this.props.user.state.rejected && this.props.user.state.reason === 'loggedout') {
      return (
        <div id='logged-out-home' className='container center-container'>
          <div className='row'>
          	<div className='col-sm-12 col-md-6'>
              <div className='jumbotron app-title'>
                <h1 className='display-2'>OddVoter</h1>
              </div>
            </div>
            <div className='col-sm-12 col-md-6'>
              <div className='jumbotron app-title'>
                <div className='row'>
                  <div className='col-sm-12 col-md-8'>
                  	<p className='lead'>Unlock your citizenship, share your voice directly with your local leaders.</p>
                  </div>
                </div>
                <div className='row'>
                  <div className='col-sm-12 col-md-8'>
                    <p className='lead'>
                      <a href="/signup" className='btn btn-primary btn-signup'>Sign up now</a>
                    </p>
                  </div>
                </div>
              </div>
    ...
    return (
      <div id='forum-home'>
        ...
        <div className='topics-container'>
          ...
          {topics.map((topic) => (
            Number(new Date(topic.closingAt)) >= Date.now() &&
  ...
  renderVoterMatchResult() {
    if (!this.state.hideVoterMatchMessage) {

      // set hideVoterMatchMessage for user to TRUE
      this.updateUserHideMessage()

      if (this.state.voterMatched) {
        return (
          <p>
            Congrats {this.state.user.firstName}! We&apos;ve found and matched your voter record! As you answer questions on the platform, your elected officials will benefit from knowing your opinions reflect a registered voter!
          </p>
        )
      } else {
        return (
