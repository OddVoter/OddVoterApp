...
export class HomeForum extends Component {
  ...
  render () {
    ...
    if (config.visibility === 'hidden' && this.props.user.state.rejected) {
      ...
      console.log('pushing signin. looks like user may not be signed in')
      return null
    }
    else if (this.props.user.state.rejected && this.props.user.state.reason === 'loggedout') {
      console.log('user is logged out. going to return blank body')
      return (
  ...
  renderVoterMatchResult() {
    if (!this.state.hideVoterMatchMessage) {
      ...
      if (this.state.voterMatched) {
        return (
          <p>We have located your voting record!</p>
        )
      } else {
        return (
