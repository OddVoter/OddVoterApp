...
import TopicReport from '../admin-topics-report/view'
...
page(urlBuilder.for('admin.topics.id.report'), privileges('canCreateTopics'), clearTagStore, findAllTags, (ctx) => {
  ctx.sidebar.set('topics')
  let form = new TopicReport(ctx.topic, ctx.forum, ctx.tags)
  form.replace('.admin-content')
  form.once('success', function () {
    topicStore.findAll({forum: ctx.forum.id})
  })
