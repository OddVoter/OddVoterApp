  ...
  constructor (topic, forum) {
    ...
    if (this.votesByMunicipalDistrictData === undefined) {
      ...
      .end((err, res) => {
        ...
        for (var data of votesByMunicipalDistrictData) {
          votesByMunicipalDistrictGrandTotal += data['no'] + data['yes'] + data['neutral']
          districts.push({ name: data['district'], grandTotal: data['no'] + data['yes'] + data['neutral'] })
        }
        const isVote = (votesByMunicipalDistrictData[0]).yes !== undefined || false

        if (!isVote) {
          const transformed = []
          for (var d of votesByMunicipalDistrictData) {
            for (var key in d) {
              console.log(key)
              transformed.push({ name: key })
            }
          }
          for (var d of votesByMunicipalDistrictData) {
