...
export default class TopicsReportView extends View {
  constructor (topic, forum) {
    const locals = {
      ...
      moment: moment,
      urlBuilder,
      registeredVoterStatusData: [],
      votesByMunicipalDistrictData: [],
      lastVotedDateData: []
    }
    ...
    console.log('Printing out the topic', this.topic)
    ...
    .end((err, res) => {
      this.registeredVoterStatusData = res.body
      console.log(res)
    })
    ...
    .end((err, res) => {
      this.votesByMunicipalDistrictData = res.body
      console.log(res)
    })
