...
app.put('/user/:id', restrict, function(req,res) {
  log('Request PUT /user/%s', req.params.id)

  api.user.update(req.params.id, req.body, function (err, user) {
    if (err) return _handleError(err, req, res)

    log('Serving user %s', user)
    res.json(api.user.expose.ordinary(user))
  })
})
