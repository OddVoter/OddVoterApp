...
const log = debug('democracyos:topic-db-api')
...
exports.list = function list (opts) {
  ...
  let query = { forum: forum._id }

  if (opts.visibility && opts.visibility === 'owner') {
    query = { forum: forum._id, owner: user._id}
  }

  forum.permissions.forEach(function(permission) {
    if (permission.user === user._id && permission.role === 'admin' || user.staff === true) {
      query = { forum: forum._id }
    }
  })
