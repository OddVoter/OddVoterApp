...
function getTopicSummaryDistrictPoll(topic, cb) {
   ...
   return async.each(topic.action.box, function (voter, done) {
       ...
       if (itemIndex !== -1) {
        ...
        // Aggregate totals per district and grand total across all districts.
        var grandTotal = 0;
        var totalsObj = { item: "TOTALS" };
        for (var i = 0; i < results.length; i++) {
            for (var key in results[i]) {
                if (results[i].hasOwnProperty(key) && key !== "item") {
                    if (totalsObj[key]) {
                        totalsObj[key] += results[i][key];
                    } else {
                        totalsObj[key] = results[i][key];
                    }
                    grandTotal += results[i][key];
                }
            }
        }

        totalsObj["grandTotal"] = grandTotal;
        results.push(totalsObj);
        cb(results)
    });
}
